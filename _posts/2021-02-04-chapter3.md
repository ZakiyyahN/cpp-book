...
The first step in debugging a code is identifying its errors. 
Run the code then you will see the corresponding error, 
then if the error you see is a syntax error, 
go to the error line and check for the misspelled keywords and syntax usage. 

If you see no error, and the output is not the expected one, 
then there might be a problem such as:

- wrong algorithm
- wrong conditions
- the flow is not continued further due to the use of:
  - semicolons, 
  - break, 
  - continue,
  - etc.
  
So, to check that, you can go into debug mode and check each step in 
the flow to see where we are doing wrong.
