...
We discussed the common errors in the previous parts.

Sometimes, when we run the program it doesn't compile successfully
and shows some errors such as *misspelled keywords* and *syntax usage*. 
In this situation, we will follow the address of the error that shows in the console and try to fix them.

![compilation error](../assets/img/book/c3/error0.png)

> The error shows that the error is located in line 7 and the 5th character.

On the other hand, Some sort of errors is not that much visible. 
The program may compile successfully but the output is not the expected one.
When we lead to this situation it means there is a problem with our code, these problems called bug.

Debugging is the process of finding bugs and resolving them. 
If we have a simple program we can easily follow the flow of the program and *hand trace* it until find the bug and correct it.
But, in the complex programs with the variety of the variables and statements; it is not easy and fast enough to do it handy.

There are some Technic and Tools that are help us in debugging.


#### Print out Step by Step!

- In each line of the code, you should know the value of each variable; if you are not sure about them, print them out.
  > For more readability, you can print the value of a variable *before* and *after* a statement. So you can see how statement change the value of the statement.

- Is it possible that a part of your code does not run at all. 
  To make sure your code got there, you can easily print a message that shows your code got there.
  ```cpp
  #include <iostream>
...
In some cases we need to ignore some statements to see the effect of the rest of the program.
We can set breakpoints such as `break` and `continue` to skip the rest of the program.


#### Run the Program After Each Change

This is not technic for debugging, this a method to make sure you don't miss the bugs in your code.
Running program after each change shows you the effect of the change in your program,
so you will never miss the bugs.


#### Use IDE's Debugger

Usually IDEs have some sort of tools for debugging which allows us to check the values of the variables and shows you the execution of the program line by line.
These utilities also let you set breakpoints through the program, modify variables, run different sections of the code separately, etc.
...
The machine that makes slushy have a small monitor that display only a number.
Barnie does not know the meaning of the numbers, so she asks you to write a program for her.
...
We learned how to write an expression for an `if-else` statement using 
