...
when the program ends, it will return a number. The number returning by `main` function
has a meaning for the operating system (OS).Mostly, OS, monitor the code, and when the program
ends by returning `0`, it can be understood that the program operates successfully.
