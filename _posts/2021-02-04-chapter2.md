...
-   The first six lines of the code is 'comment block' which makes a brief introduction to C++ program.
...
-   The following two lines tell the compiler to add **console** input and output (`iostream`) features to the program and make them easier to use by adding their namespace. It will probably be used in every program this semester.

-   Line 8 is what is known as a **preprocessor command**. The pound (`#`)
...
-   'nt main() {:.cpp}`'  is the executable part of our program. The function name is  `main,` and it returns an integer type value which its function, as indicated by the empty parentheses, has no parameters.
...
-   '`cout`' is the command to print text or data **on the screen**.
    Anything enclosed in **quotation marks** is to be printed.

-   '`cin`' - command  receives data from the end user.
