...
> You can find out more [here](https://betterprogramming.pub/string-case-styles-camel-pascal-snake-and-kebab-case-981407998841).
...
For non-boolean operands, the expression `++x` is exactly equivalent to `x += 1` or `x = x+1`, 
and the expression. `--x` is exactly equivalent to `x -= 1` or `x = x-1`, 
...
For example, `intValue = 10.66;` and `intValue = int(10.66);` produce the same result `10`.
