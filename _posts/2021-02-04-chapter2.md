...
Reading input promotes the interaction with the user and helps to get the necessary information for the program. Imagine writing a program to calculate the area of a rectangle with a length of `x` and width of `y`. The very first step to calculate the area is to get the length and width from the input. 
