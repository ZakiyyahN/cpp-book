...
Let us start our journey by learning some basic concepts and fundamentals of programming. We will learn about simple inputs/outputs, identifiers, variables, and related expressions in this chapter. These are some crucial tools that will help the information to be circulated around a user and code. Additionaly, We will learn:
1. How to get the information from the user
2. How to save that information
3. How to do some calculation using that information
and;
4. How to let the user know about the result of the calculation


## 2.2 Writing a Simple Program

Examining the following C++ program,
...
-   The first 19 lines of the code is 'comment block' which makes a brief introduction to C++ program.

-   By convention, every single C++ program is better to have this kind of
...
-   Typically, there is an inline comment at the beginning or end of the processing block, such as `//` at the end of the program.

-   The following two lines(21 and 23) tell the compiler to add **console** input and output (`iostream`) features to the program and make them easier to use by adding their namespace. It will probably be used in every program this semester.

-   Line 21 is what is known as a **preprocessor command**. The pound (`#`)
    sign followed by `include < >` begins every preprocessor command.

-   Any **built-in** or **user-defined** header files using in the program must follow this same syntax.

> **NOTE**: 
>
> Built-in header files, in the program, are enclosed inside the `<` and `>` marks, and created header files in `""` quotation marks.

-   `int main() { }`  is the executable part of our program. The function name is  `main`, and it returns an integer type value which its function, as indicated by the empty parentheses, has no parameters.

-   The parentheses `{ }` mark the **beginning** and **end**
    of a group(scope) of C++ statements. In this case, they enclose our `main`
    function.

-   `int number;` sets `number` as a variable of input type **integer**.

-   `cout` is the command to print out text or data **on the screen**.
    Anything enclosed in **quotation marks**(string) is to be printed.
...
-   `return 0;` - This line is there, since the return value in
    ...
    compiler from reporting a syntax error. Also `0` means the program finished successfully.

-   `// end of the program` - a single line comment.

## 2.3 Reading Input from the Keyboard

Reading input promotes the interaction with the user and helps to gain the necessary information for the program. Imagine writing a program to calculate the area of a rectangle with a length of `x` and width of `y`. The very first step to calculate the area is to get the length and width from the input. 

Working knowledge of C++ Basic I/O is an important first stage in learning C++.
Here is the table of C++ **basic Input/Output** (I/O) symbols we saw in the preceding program:
...
For instance, getting `x`{:.cpp} (length) and `y`{:.cpp} (width) from input, we could
say:
```cpp
cin >> x;
cin >> y;
...
>    You **cannot** use them as your user defined identifier such as variable names.<br>
...
The rest of the letters are lower case.<br> 
For example, `payRate`, `camelCase`, and `numberOfYears` are `camelCase`.

![CamelCase](../assets/img/book/c2/CamelCase.png)

> Also there are other ways to write phrases such as `PascalCase`, `snake_case`, `kebab-case`.
> <br>
...
> NB: A variable can be used only **after its value is set**.


## 2.6 Assignment Statements and Assignment Expressions

First, You need to know the definition of an **arithmetic expression** and the **precedence** of the operators.
...
The initialized variable can be declared as a **constant** by adding the type qualifier `const` before the definition. 
The general format for a const declaration is shown below:
...
In the preceding program, we used the data type `int,` an identifier for the integer data type.

We used data type `int` in four ways in the preceding program. 
`int` precedes `main,` the name of the `main` function in the program to indicate that the function returns the value of type `int.` 
In the end, an `int`, literal (zero) is returned as the result of the `main` function `main`. 
...
That is, the order of execution of an expression that contains more than one operation is determined by the precedence rules of arithmetic. 
...
The Postfix increment and decrement operators have the highest precedence over any of the arithmetic operators.
...
THIS IS A COMMENT, SECTION LIKE THIS WON'T DISPLAY IN WEBSITE!!!
...
that is, the prefix increment or decrement is an lvalue expression that identifies the modified operand. 

![](https://res.cloudinary.com/practicaldev/image/fetch/s--jb5VUvnN--/c_imagga_scale,f_auto,fl_progressive,h_420,q_auto,w_1000/https://thepracticaldev.s3.amazonaws.com/i/dj6nqlncmujz30ozru22.png)
...
> <br>
> Note: both printf statements are separate not one after other.


All arithmetic conversion rules and pointer arithmetic rules defined for arithmetic operators apply and determine
the implicit conversion (if any) applied to the operand as well as the return type of the expression.
...
the integral value is changed **temporarily** to its equivalent floating point representation before the operation is executed. 
...
For example, `int value = 10.66;` and `int value = int(10.66);` and `int value = (int)10.66;` produce the same result `10`.
...
In software engineering, a software development process is the process of dividing software development work into 
