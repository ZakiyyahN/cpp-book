...
To write a program, we involve in designing a problem-solving strategy, then implement 
that strategy using a programming language.


**Examining the following C++ program,** <br> 
_**Writing a Simple Program**_
...
-   The following two lines ,21 and 23, tell the compiler to add 
    **console** input and output (`iostream`) features to the program 
    ...
    It will probably be used in every program this semester.
-   Line 21 is what is known as a **preprocessor command**. The pound (`#`)
    sign followed by `include < >` begins every preprocessor command.

-   Any **built-in** or **user-defined** header files using in the program 
    must follow this same syntax.
...
-   `return 0;` - This line is there, since the return value in
    ...
    compiler from reporting a syntax error. Also `0` means the program 
...
## 2.3 Identifiers
_Identifiers_ are the names that identify elements such as variables and functions in program.
...
> **NOTE**: 
> 
> 1. _Reserved words_ are certain words which have predefined meanings within the C++
>    language. <br> 
>    
...
**camelCase**:
_Camel case_ is the practice of writing phrases without spaces or punctuation, 
...
> NB: A _variable_ can be used only **after its value is set**.


## 2.6 Assignment Statements and Assignment Expressions

It designates a value for a variable, and it can be used as an expression in C++ . When we declare a variable, 
we can assign a value to it by using an _assignment statement_. At the outset, You 
need to know the definition of an _arithmetic expression_ and the _precedence_ of the 
operators.


_Arithmetic Expressions_ are defined as Variables and constants of integral and floating-point types can be combined
into expressions using arithmetic operators.


## 2.7 Named Constants

The initialized variable can be declared as a _constant_ by adding the type qualifier `const` before 
the definition. The general format for a `const` declaration is shown as below:
...
In the preceding program, we used the data type `int,` an identifier for the integer
 data type.
...
that is, the prefix increment or decrement is a lvalue expression that identifies the modified operand. 
...
Coercion from an integer to a floating point is _exact._ 
...
type before it. This is called _type casting_ or _type conversion_. For example, <br> <br>
...
</br>Conversion function is declared like a non-static member function or member function template 
with no parameters, no explicit return type, and with the name of the form:
...
represent an array  a function or type.
...
development, and extreme programming. <br> <br> Some methodologies are sometimes known as software development life 
